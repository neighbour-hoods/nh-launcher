      ...
      >
        ${this.loading
          ? html`
            ...
            >
              <span style="opacity: 0;"><slot></slot></span>
              <sl-spinner
                style="position: absolute; left: calc(50% - 0.75rem); font-size: 1.5rem; --track-width: 3px; --track-color: var(--nh-theme-accent-default); --indicator-color: var(--nh-theme-accent-subtle);"
              ></sl-spinner>
            </div>`
          : html`
            <div class="button-inner">
              ${this.iconImageB64
                ? html`<img
                      alt="button icon"
                      src=${`data:image/svg+xml;base64,${this.iconImageB64}`}
                    /><slot></slot>`
